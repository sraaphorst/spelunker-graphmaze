...
namespace spelunker::typeclasses {
    ...
    /// This is particularly important since the Boost Graph Library represents vertex_descriptors as unsigned long.
    template<>
    struct Show<unsigned long> {
        static std::string show(const unsigned long i) {
            return std::to_string(i);
        }
